import { HOUSE, type CasinoState, type CasinoPayout, type Owner } from "./types";

/**
 * Official payout algorithm for one casino once all dice are placed:
 * 1. Any owners tied at the exact same dice count are eliminated entirely (no payout) — this
 *    applies at every count level, not just the top.
 * 2. Surviving owners are ranked by descending dice count.
 ...
 * 4. A bill awarded to the neutral "house" pseudo-player is discarded (recycled), never paid out.
 */
export function resolveCasino(casino: CasinoState): CasinoPayout {
  const entries = Object.entries(casino.diceCounts).filter((entry): entry is [Owner, number] => {
    const [, count] = entry;
    return typeof count === "number" && count > 0;
  });

  const byCount = new Map<number, Owner[]>();
