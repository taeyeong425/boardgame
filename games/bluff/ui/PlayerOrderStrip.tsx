      ...
      <div className="flex gap-1.5 overflow-x-auto p-1">
        {turnOrder.map((id, i) => {
          ...
          return (
            <div
              key={id}
              className={`flex min-w-20 shrink-0 flex-col items-center gap-0.5 rounded-lg border px-2 py-1.5 text-xs ${
                isCurrent
            ...
            >
              ...
              <span className="font-semibold">{isSelf ? "나" : (playerNames[id] ?? "?")}</span>
              <span className="text-white/70">{eliminated ? "탈락" : `🎲 ${diceCount}`}</span>
            </div>
          );
        })}
      </div>
